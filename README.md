...
- API 供应商站点可在插件配置或 API Key 弹窗中设置 `image_generation_path` / `image_edit_path`，分别覆盖纯文本生图和参考图编辑接口路径；留空默认 `/images/generations` / `/images/edits`。客户端仍可统一请求 `/api/gateway/v1/images/generations` 或 `/api/gateway/v1/images/edits`，网关会按命中的站点配置转到对应上游路径。
